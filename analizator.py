...
"""
    ...
    def draw_sentiment_bar(self, score):
        ...
        # Подписи
        self.sentiment_bar.create_text(10, height/2, text="-1 (негатив)", anchor=tk.W, fill='#666666', font=('Arial', 8))
        self.sentiment_bar.create_text(width-10, height/2, text="+1 (позитив)", anchor=tk.E, fill='#666666', font=('Arial', 8))
        
        # Текущее значение
        if abs(score) > 0.01:  # Не показываем для нулевых значений
            value_x = center_x + (score * width / 2)
            self.sentiment_bar.create_oval(value_x-4, height/2-4, value_x+4, height/2+4, fill='white', outline='#333333', width=2)
